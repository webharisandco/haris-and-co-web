...
<header className="fixed top-0 left-0 w-full z-50 flex justify-between  bg-black px-[18px] py-[40px] md:px-[80px] lg:px-[95px] md:py-[40px] items-center border-b border-b-white/50">
      ...
      <div
        className={
          "hidden lg:flex justify-between items-center gap-[30px] xl:gap-[40px] text-white/80 *:font-light  "
        }
      >
        <Link to="/" className="text-[18px] font-[AbroRegular] text-white">
          Home
        </Link>
        <Link to="/services" className="text-[18px] font-[AbroRegular] ">
          Services
        </Link>
        <Link to="/casestudies" className="text-[18px] font-[AbroRegular]">
          Case Studies
        </Link>
        <Link to="/career" className="text-[18px] font-[AbroRegular]">
          Career
        </Link>
        <Link to="/blog" className="text-[18px] font-[AbroRegular]">
          Blog
        </Link>
        <Link to="/contact" className="text-[18px] font-[AbroRegular]">
          Contact Us
        </Link>
      </div>
      <LandingButton
      ...
      <div className="lg:hidden flex ms-auto max-w-fit">
        <button onClick={toggleMenu}>
          ...
          >
            <span
              className={`block h-[2px] w-6 md:w-9 bg-white rounded-sm transition-transform duration-300 ${
                menuOpen ? "translate-y-[7px] rotate-45" : ""
              }`}
            ></span>
            <span
              className={`block h-[2px] w-5 md:w-[30px] bg-white rounded-sm transition-opacity duration-300  ${
                menuOpen ? "opacity-0" : ""
              }`}
            ></span>
            <span
              className={`block h-[2px]  bg-white rounded-sm transition-transform duration-300 ${
                menuOpen
                  ? "-translate-y-[5px] md:-translate-y-[7.5px] -rotate-45 w-6 md:w-9"
                  : "w-4 md:w-6"
              }`}
            ></span>
          </button>
