  ...
  return (
    <div className="bg-white flex flex-col lg:items-center lg:flex-row pt-[18px] lg:pt-0 justify-between   items-center px-[16px] lg:px-[80px] min-h-[632px]">
      ...
      <div className=" w-full lg:w-fit my-[30px] lg:my-0">
        <ul className="text-black/50 font-[thin] text-[24px] text-start lg:text-end flex flex-col gap-[12px] 3xl:gap-5">
          {menuItems.map((item) => (
            <Link to={item.link}>
              <li
                key={item.id}
                onClick={() => handleClick(item.id, item.content)}
                className={`cursor-pointer hover:text-black ${selectedId === item.id ? "text-black" : "text-black/50"
                  }`}
              >
                {item.label}
              </li>
